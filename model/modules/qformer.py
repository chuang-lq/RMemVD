...
class QuadrangleAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 window_size=8, rpe='v2', coords_lambda=20):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = qk_scale or head_dim ** -0.5  # 避免softmax的两端化
    ...
    def forward(self, x, x_size):
        ...
        image_reference = (torch.stack(torch.meshgrid(image_reference_w, image_reference_h), 0)
                           .permute(0, 2, 1).unsqueeze(0))  # 2, h, w
        ...
        window_coords = (torch.stack(torch.meshgrid(expanded_base_coords_w, expanded_base_coords_h), 0)
                         .permute(0, 2, 1).reshape(1, 2, window_num_h, ws, window_num_w, ws).permute(0, 2, 4, 1, 3, 5))
        ...
        assert not torch.isnan(attn).any(), "quad-attn contains NaN!"
        ...
        out = self.proj(out)
        assert not torch.isnan(out).any(), "QuadrangleAttention contains NaN!"
        return out
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=8, shift_size=0,
        ...
        self.attn = QuadrangleAttention(
            ...
            window_size=window_size, rpe=rpe, coords_lambda=coords_lambda)
    ...
    def forward(self, x):
        ...
        shortcut = x
        x = self.norm1(x)
        x = self.attn(x, self.input_resolution)
        x = x.view(B, H * W, C)
...
class RQB(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        # x_size: H, W
        H, W = x_size
        res = self.residual_group(x)  # B, H*W, C
        if self.downsample is not None:
            res = rearrange(res, 'B (H W) C -> B C H W', H=H//2, W=W//2)
        else:
...
class RQBs(nn.Module):

    # img_size: (list/tuple[int])
    def __init__(self, img_size, dim, depth=4, window_size=8, qkv_bias=True,
                 norm_layer=nn.LayerNorm, rpe='v1', coords_lambda=0):
        super(RQBs, self).__init__()

        self.rqb1 = RQB(dim=dim, input_resolution=(img_size[0] // 2, img_size[1] // 2),
        ...
        self.rqb2 = RQB(dim=dim * 2, input_resolution=(img_size[0] // 4, img_size[1] // 4),
    ...
    def forward(self, x):
        ...
        x = self.rqb1(x, x_size=(H, W))
        x = self.rqb2(x, x_size=(H // 2, W // 2))
...
if __name__ == '__main__':
    ...
    model = RQBs(img_size=[256, 256], dim=64, depth=2, window_size=8).cuda()
    x = model(x)
    print(x.shape)
