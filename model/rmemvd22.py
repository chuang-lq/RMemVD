...
from .modules import *
from .memory_bank2 import MemoryBank
...
class Model(nn.Module):
    ...
    def forward(self, inputs, profile_flag=False):
        ...
        for i in range(t - 1, -1, -1):
            ...
            key_curr, shrinkage, selection, encoder_outs, semantic_f = self.memory.encode_key(down_feat, encoder_outs, decoder_outs)
            ...
            if i == t - 1:
                ...
                pixel_readout, object_readout = self.memory.mem_bank_backward.match_memory(key_curr, selection, semantic_f)
            ...
            self.memory.mem_bank_backward.add_memory(key_curr, values=[pixel, object_feat], shrinkage=shrinkage)
        ...
        for i in range(0, t):
            ...
            if i == 0:
                ...
                pixel_readout, object_readout = self.memory.mem_bank_forward.match_memory(key_curr, selection, semantic_f)
                object_readout, pixel_readout = self.object_transformer(object_readout, pixel_readout)
                hidden = self.memory.mem_bank_forward.get_hidden()
                hidden, decoder_outs = self.memory.decoder(f_2, f_1, f, semantic_f, pixel_readout, object_readout, hidden)
                # down_feat = self.feat_fusion(down_feat, down_feats[:, i-1])
            ...
            self.memory.mem_bank_forward.add_memory(key_curr, values=[pixel, object_feat], shrinkage=shrinkage)
...
class Memory(nn.Module):
    ...
    def encode_key(self, frame, encoder_outs=None, decoder_outs=None, need_sk=True, need_ek=True):
        ...
        return k, shrinkage, selection, encoder_outs, semantic_f


def cost_profile(model, H, W, seq_length=1):
...
if __name__ == '__main__':
    x = torch.randn(2, 8, 3, 64, 64).cuda()
    model = Model().cuda()
    x = model(x)
    print(x.shape)
